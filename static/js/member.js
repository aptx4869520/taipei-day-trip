...
const memberHeaders = memberToken ? { Authorization: `Bearer ${memberToken}` } : {};
async function loadMember() {
  try {
    ...
    document.querySelector("#mcp-host").textContent = `${window.location.origin}/mcp/`;
...
document.querySelector("#generate-token").addEventListener("click", async (event) => {
  const button = event.currentTarget;
  const status = document.querySelector("#token-status");
  button.disabled = true;
  status.textContent = "正在更新金鑰…";
  try {
    const response = await fetch("/api/member/token", { method: "POST", headers: memberHeaders, cache: "no-store" });

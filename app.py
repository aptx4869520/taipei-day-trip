...
from member_tokens import rotate_member_token
from trip_services import BookingInput, format_attraction, get_attractions, save_booking
...
async def generate_member_token(request: Request):
    user_id = get_authenticated_user_id(request)
    if user_id is None:
        return JSONResponse(status_code=403, content={"error": True, "message": "未登入系統，拒絕存取"})
    try:
        token = rotate_member_token(user_id)
        return JSONResponse(content={"token": token}, headers={"Cache-Control": "no-store"})
    except Exception as error:
        print("Member token update failed:", type(error).__name__)
        return JSONResponse(status_code=500, content={"error": True, "message": "金鑰更新失敗，請稍後再試"})
...
async def attractions_api(page: int = Query(..., ge=0), keyword: str | None = None, category: str | None = None):
    return await get_attractions(page, keyword, category)
...
async def create_booking(request: Request, booking_data: BookingInput):
    user_id = get_authenticated_user_id(request)
    if user_id is None:
        return JSONResponse(status_code=403, content={"error": True, "message": "未登入系統，拒絕存取"})
    return await save_booking(user_id, booking_data)
